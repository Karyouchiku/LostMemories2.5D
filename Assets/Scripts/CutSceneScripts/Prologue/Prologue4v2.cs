...
public class Prologue4v2 : MonoBehaviour, CutScenes, ISaveable//Rename Class ***********************
{
    ...
    IEnumerator TeleportTolocationCoroutine()
    {
        ...
        yield return new WaitForSeconds(1);
        SetActorStartingPosition(0, 5);
        SetActorStartingPosition(2, 6);
    ...
    IEnumerator LoopingTransition(int i)
    {
        transition.ManualTransitionON();
        yield return new WaitForSeconds(1);
        SetActorStartingPosition(0, 0);
        SetActorStartingPosition(2, 1);
